...
/***/function (module, exports) {
		...
		return function (ratio) {
...

			{
				...
				lb.data.forEach(function (entry) {
					...
					var scale = _height4 / entry.position.axis,
					    xx = _x37 - 1.5 * _height4 - scale * entry.position.middle.x * 0.707,
					    yy = _y10 + 0.5 * _height4 + scale * entry.position.middle.x * 0.707;
					drawEntity(xx, yy, entry.image, 1 / scale, 1, scale * scale / entry.image.size, -Math.PI / 4, true);
					// Move down
					_y10 += _vspacing3 + _height4;
				});
			}
			...
			//Draw upgrade menu
			{
				...
				if (_gui.upgrades.length > 0) {
					...
					var internalSpacing = 6;
					var _len11 = alcoveSize * global.screenWidth / 2 * 1;
					var _height5 = _len11;
					var _x38 = glide * 2 * spacing - spacing;
					var _y11 = spacing;
					var xo = _x38;
					var xxx = 0;
					var yo = _y11;
					...
					_gui.upgrades.forEach(function drawAnUpgrade(model) {
						if (_y11 > yo) yo = _y11;
						xxx = _x38;
						global.clickables.upgrade.place(_i27++, _x38, _y11, _len11, _height5);
						// Draw box
						ctx.globalAlpha = 0.5;
						ctx.fillStyle = getColor(colorIndex);
						drawGuiRect(_x38, _y11, _len11, _height5);
						ctx.globalAlpha = 0.1;
						ctx.fillStyle = getColor(-10 + colorIndex++);
						drawGuiRect(_x38, _y11, _len11, _height5 * 0.6);
						ctx.fillStyle = color.black;
						drawGuiRect(_x38, _y11 + _height5 * 0.6, _len11, _height5 * 0.4);
						ctx.globalAlpha = 0.9;
						...
						var picture = getEntityImageFromMockup(model, _gui.color),
						    ...
						    xx = _x38 + 0.5 * _len11 - scale * position.middle.x * Math.cos(upgradeSpin),
						    yy = _y11 + 0.5 * _height5 - scale * position.middle.x * Math.sin(upgradeSpin);
						drawEntity(xx, yy, picture, 1, 1, scale / picture.size, upgradeSpin, true);
						// Tank name
						text.upgradeNames[_i27 - 1].draw(picture.name, _x38 + _len11 / 2, _y11 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'center');
						ctx.strokeStyle = color.black;
						ctx.globalAlpha = 1;
						ctx.lineWidth = 3;
						drawGuiRect(_x38, _y11, _len11, _height5, true); // Border
						if (_ticker++ % 2) {
							_y11 -= _height5 + internalSpacing;
							_x38 += glide * (_len11 + internalSpacing);
						} else {
							_y11 += _height5 + internalSpacing;
						}
					});
	...
	var gameDrawDead = function () {
		...
		var getKills = function getKills() {
			...
			return (b === 0 ? '🌼' : b < 4 ? '🎯' : b < 8 ? '💥' : b < 15 ? '💢' : b < 25 ? '🔥' : b < 50 ? '💣' : b < 75 ? '👺' : b < 100 ? '🌶️' : '💯') + (finalKills[0] || finalKills[1] || finalKills[2] ? ' ' + (finalKills[0] ? finalKills[0] + ' kills' : '') + (finalKills[0] && finalKills[1] ? ' and ' : '') + (finalKills[1] ? finalKills[1] + ' assists' : '') + ((finalKills[0] || finalKills[1]) && finalKills[2] ? ' and ' : '') + (finalKills[2] ? finalKills[2] + ' bosses you killed' : '') : 'Peaceful boi') + '.';
		};
		...
		return function () {
			...
			text.taunt.draw('Congratulations, you died', x, y - 80, 11, color.guiwhite, 'center');
			text.level.draw('Tank Level = ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name + '.', x - 170, y - 30, 24, color.guiwhite);
			text.score.draw('Your Score: ' + formatLargeNumber(Math.round(global.finalScore.get())), x - 170, y + 25, 50, color.guiwhite);
			text.time.draw('Survied for: ' + timeForHumans(Math.round(global.finalLifetime.get())) + '.', x - 170, y + 55, 16, color.guiwhite);
			text.kills.draw(getKills(), x - 170, y + 77, 16, color.guiwhite);
			text.death.draw(getDeath(), x - 170, y + 99, 16, color.guiwhite);
			text.playagain.draw('Enter = Respawn, Reload = Main Menu', x, y + 125, 16, color.guiwhite, 'center');
		};
	}();
	...
	var gameDrawDisconnectedlolwut = function () {
		var text = {
			disconnected: TextObj(),
			message: TextObj()
		};
	...
	function animloop() {
		...
		if (global.TFmanlolwut) {
			gameDrawDisconnectedlolwut();
		}
	}
